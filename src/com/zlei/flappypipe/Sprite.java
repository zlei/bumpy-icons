...
public abstract class Sprite {
	...
	/**
	 * Counter for the frames Cycling through the columns
	 */
	...
	/**
	 * Draws the frame of the bitmap specified by col and row at the position
	 * given by x and y
	 * 
	 * @param canvas
	 *            Canvas that should be drawn on
	 */
	public void draw(Canvas canvas) {
		src = new Rect(col * width, row * height, (col + 1) * width, (row + 1)
				* height);
	...
	public void move() {
		// changeToNextFrame();
		// Its more efficient if only the classes that need this implement it in
		// their move method.
	...
	/**
	 * Checks whether this sprite is so far to the left, it's not visible
	 * anymore.
	 * 
	 * @return
	 */
	...
	/**
	 * Checks whether the sprite is touching this. Seeing the sprites as
	 * rectangles.
	 * 
	 * @param sprite
	 * @return
	...
	/**
	 * Checks whether the sprite is touching this. With the distance of the 2
	 * centers.
	 * 
	 * @param sprite
	 * @return
	...
	/**
	 * Checks whether the point specified by the x and y coordinates is touching
	 * the sprite.
	 * 
	 * @param x
	 * @param y
	...
	public boolean isTouchingGround() {
		return this.y + this.height > this.view.getHeight()
				- this.view.getHeight() * Frontground.GROUND_HEIGHT;
	}
	...
	/**
	 * Creates a bitmap that is scales with the getScaleFactor method. Static so
	 * everyone, who provides a Context, can use it.
	 * 
	 * @param drawable
	 * @return
	 */
	public static Bitmap createBitmap(Drawable drawable, Context context) {
		...
		return Bitmap.createScaledBitmap(bm,
				...
				(int) (bm.getHeight() * getScaleFactor(context)), false);
	}

	/**
	 * Returns a scale factor related to the screen resolution that is used for
	 * scaling bitmaps.
	 * 
	 * @param context
	 * @return
	...
	/**
	 * Gives a value that will be tolerated when touching a sprite. Because my
	 * images have some whitespace to the edge.
	 * 
	 * @return
	 */
