...
public class Background extends Sprite {
	...
	/**
	 * Draws the bitmap to the Canvas. The height of the bitmap will be scaled
	 * to the height of the canvas. When the bitmap is scrolled to far to the
	 * left, so it won't cover the whole screen, the bitmap will be drawn
	 * another time behind the first one.
	 */
	@Override
	public void draw(Canvas canvas) {
		...
		int endBitmap = Math.min(-x + (int) (canvas.getWidth() / factor),
				bitmap.getWidth());
		...
		if (endBitmap == bitmap.getWidth()) {
			// draw second bitmap
			src = new Rect(0, 0, (int) (canvas.getWidth() / factor),
					bitmap.getHeight());
			dst = new Rect(endCanvas, 0, endCanvas + canvas.getWidth(),
					canvas.getHeight());
			canvas.drawBitmap(this.bitmap, src, dst, null);
		}
	}
