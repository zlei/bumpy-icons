...
public class QLearning {
	 ...
	 * @param players
	 * @param obstacles
	 * vertical_distance -844 to 524
	 * horizontal_distance 0 to 910
	 */
	public static void learnAndPerform(List<PlayableCharacter> players, List<Obstacle> obstacles) {
		for (PlayableCharacter player : players) {
			...
			for (Obstacle obs : obstacles) {
				...
				if (pipe.getX() + pipe.getWidth() >= player.getX()) {
					int diff = (pipe.getX() - player.getX());
			...
			// Step 3: Update Q(S, A)
			int state_bin_v = (int) Math.max(Math.floor((player.m_state[0] - PlayableCharacter.vertical_dist_range[0])
							/ PlayableCharacter.resolution), 0);

			int state_bin_h = (int) Math.max(Math.floor((player.m_state[1] - PlayableCharacter.horizontal_dist_range[0])
									/ PlayableCharacter.resolution), 0);

			int state_dash_bin_v = (int) Math.max(Math.floor((player.m_state_dash[0] - PlayableCharacter.vertical_dist_range[0])
									/ PlayableCharacter.resolution), 0);

			int state_dash_bin_h = (int) Math.max(Math.floor((player.m_state_dash[1] - PlayableCharacter.horizontal_dist_range[0])
									/ PlayableCharacter.resolution), 0); 
			...
			if (Math.random() < PlayableCharacter.explore) {
				int rand = (Math.random() < 0.5) ? 0 : 1;
				player.action_to_perform = rand == 0 ? 0 : 1;
			} else {
				state_bin_v = (int) Math.max(Math.floor((player.m_state[0] - PlayableCharacter.vertical_dist_range[0])
										/ PlayableCharacter.resolution), 0);

				state_bin_h = (int) Math.max(Math.floor((player.m_state[1] - PlayableCharacter.horizontal_dist_range[0])
										/ PlayableCharacter.resolution), 0);
	...
	public static void learnAndPerformForPipe(List<PlayableCharacter> players, List<Obstacle> obstacles) { 
		for (PlayableCharacter player : players) {
			int reward = -10;
			if (player.isDead) 
				reward = 10;
			...
			double max = -99999;
			for (int i=0; i<10; i++) {
				double temp = QLearning.Q_pipe[state_dash_bin_v][state_dash_bin_h][i];
				if (temp > max)
					max = temp; 
			}
			double Q_s_a = QLearning.Q_pipe[state_bin_v][state_bin_h][pipe.action_to_perform];
			QLearning.Q_pipe[state_bin_v][state_bin_h][pipe.action_to_perform] = Q_s_a
					+ pipe.alpha_QL
					* (reward + max - Q_s_a);
